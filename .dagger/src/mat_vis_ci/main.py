...
class MatVisCi:
    ...
    @function
    async def build_thumbnail_packs(
        self,
        src: Annotated[dagger.Directory, Doc("Project root directory")] | None = None,
        source: Annotated[str, Doc("Source name")] = "ambientcg",
        tier: Annotated[str, Doc("Resolution tier")] = "1k",
        tag: Annotated[str, Doc("Release tag")] = "v0000.00.0",
    ) -> dagger.Directory:
        """Run baker, then zip thumbnails per source x size.

        Returns a directory containing:
        - mat-vis-<source>-thumbnails-128.zip
        - mat-vis-<source>-thumbnails-256.zip
        - mat-vis-<source>-thumbnails-512.zip
        """
        context = src or dag.host().directory(".")
        pip_cache = dag.cache_volume("pip-cache")

        # Run the baker to generate thumbnails
        ctr = (
            dag.container()
            .from_("python:3.12-slim")
            .with_exec(["apt-get", "update", "-qq"])
            .with_exec(["apt-get", "install", "-y", "-qq", "zip"])
            .with_mounted_cache("/root/.cache/pip", pip_cache)
            .with_mounted_directory("/app", context)
            .with_workdir("/app")
            .with_exec(["pip", "install", "--quiet", "-e", ".[baker]"])
        ...
        # Zip thumbnails by size
        zip_script = f"""\
#!/bin/sh
set -e
THUMB_DIR="/tmp/out/mtlx/{source}"
OUT="/tmp/zips"
mkdir -p "$OUT"
for size in 128 256 512; do
    ZIPFILE="$OUT/mat-vis-{source}-thumbnails-${{size}}.zip"
    cd /tmp/out/mtlx
    find "{source}" -name "*_thumb_${{size}}.png" | sort > /tmp/filelist.txt
    if [ -s /tmp/filelist.txt ]; then
        zip -q "$ZIPFILE" -@ < /tmp/filelist.txt
    else
        echo "WARN: no thumbnails found for size $size"
    fi
done
ls -lh "$OUT"/
"""
        ctr = ctr.with_new_file("/tmp/zip_thumbs.sh", contents=zip_script, permissions=0o755)
        ctr = ctr.with_exec(["sh", "/tmp/zip_thumbs.sh"])

        return ctr.directory("/tmp/zips")
