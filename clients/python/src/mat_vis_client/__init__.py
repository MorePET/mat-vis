"""mat-vis-client — pure Python client for mat-vis PBR textures.
...
Module-level convenience API:
    ...
    # Format adapters (generic: scalars dict + textures dict)
    threejs_dict = to_threejs(scalars, textures)
...
def seed_indexes(client: MatVisClient) -> None:
    ...
    Workaround for indexes not being in git yet — they ship as
    release assets. Tries the current release tag first, falls back
    to older tags.
    """
    ...
    for source in sources:
        ...
        for t in tags_to_try:
            ...
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "mat-vis-client"})
